...
----------------------------------------------------------------- PUBLIC
----------------------------------------------------- Méthodes publiques
void DecoderDecode(ifstream & log, int time, int ext){
int i=0;
	string mot ;
	getline(log, mot);
	while(log.peek() != EOF){
		...
		if ( ((ext && !extension(d.target))  !ext) && ( (time0  time23)  ( (time=0 && time=23) && d.heure==time) ) ){
		infos.push_back(d);
		}
		getline(log,mot);
		i++;
...
------------------------------------------------------------------ PRIVE
string Decoderextract(string & mot, char delim){
...
void Decoderclean(string & url){
if(!strcmp(httpintranet-if.insa-lyon.fr,url.substr(0,31))){
if(!url.substr(0,31).compare(httpintranet-if.insa-lyon.fr)){
url =url.substr(31);
}
int i = url.find('');
url = url.substr(0,i);
...
int Decoderextension(string target){
int i = target.find('.');
